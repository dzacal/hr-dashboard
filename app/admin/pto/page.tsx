    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mb-8">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              {['Employee', 'From', 'To', 'Days', 'Reason', 'Submitted', 'Actions'].map(h => (
          ...
          <tbody className="divide-y divide-slate-50">
            ...
            {pending.map((r: any) => (
              <tr key={r.id} className="hover:bg-slate-50">
                ...
                <td className="px-5 py-3 text-slate-600">{r.days_requested}</td>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              {['Employee', 'From', 'To', 'Days', 'Status', 'Submitted'].map(h => (
                <th key={h} className="text-left px-5 py-3 text-slate-600 font-semibold text-xs uppercase tracking-wide">{h}</th>
              ))}
            </tr>
          </thead>
